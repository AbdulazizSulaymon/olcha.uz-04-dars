import logo from './logo.svg';
import './App.css';

function App() {
  return (
    <div>
      <header className="shadow">
        <div className="container-fluid py-4">
          <img src="data:image/png;base64," alt="" />
        </div>
      </header>

      <section id="products" className="py-3">
        <div className="container-fluid">
          <div className="row">
            <div className="col-sm-6 col-md-4 col-lg-3 productCol">
              <div className="productCard p-3">
                <img className="d-block productImg" src="https://olcha.uz/image/200x200/products/OlO1yD3pLX3Gtmr4M4OxyulsWnzoJXDvPYz8nQGv2rNMBTlA7NzBgGY3m6vN.jpeg" alt="" />
                <a href="#" className="groupName d-block mb-2 mt-3">Смартфоны</a>
                <p className="price mb-2">2  290  000 сум  </p>
                <p className="priceMonth">222  000 сум / мес.</p>
                <p className="title">Xiaomi Redmi 10 4/128ГБ Sea Blue</p>
                <a href="#" className="btn">Купить</a>
              </div>
            </div>
            <div className="col-sm-6 col-md-4 col-lg-3 productCol">
              <div className="productCard p-3">
                <img className="d-block productImg" src="https://olcha.uz/image/200x200/products/OlO1yD3pLX3Gtmr4M4OxyulsWnzoJXDvPYz8nQGv2rNMBTlA7NzBgGY3m6vN.jpeg" alt="" />
                <a href="#" className="groupName d-block mb-2 mt-3">Смартфоны</a>
                <p className="price mb-2">2  290  000 сум  </p>
                <p className="priceMonth">222  000 сум / мес.</p>
                <p className="title">Xiaomi Redmi 10 4/128ГБ Sea Blue</p>
                <a href="#" className="btn">Купить</a>
              </div>
            </div>
            <div className="col-sm-6 col-md-4 col-lg-3 productCol">
              <div className="productCard p-3">
                <img className="d-block productImg" src="https://olcha.uz/image/200x200/products/OlO1yD3pLX3Gtmr4M4OxyulsWnzoJXDvPYz8nQGv2rNMBTlA7NzBgGY3m6vN.jpeg" alt="" />
                <a href="#" className="groupName d-block mb-2 mt-3">Смартфоны</a>
                <p className="price mb-2">2  290  000 сум  </p>
                <p className="priceMonth">222  000 сум / мес.</p>
                <p className="title">Xiaomi Redmi 10 4/128ГБ Sea Blue</p>
                <a href="#" className="btn">Купить</a>
              </div>
            </div>
            <div className="col-sm-6 col-md-4 col-lg-3 productCol">
              <div className="productCard p-3">
                <img className="d-block productImg" src="https://olcha.uz/image/200x200/products/OlO1yD3pLX3Gtmr4M4OxyulsWnzoJXDvPYz8nQGv2rNMBTlA7NzBgGY3m6vN.jpeg" alt="" />
                <a href="#" className="groupName d-block mb-2 mt-3">Смартфоны</a>
                <p className="price mb-2">2  290  000 сум  </p>
                <p className="priceMonth">222  000 сум / мес.</p>
                <p className="title">Xiaomi Redmi 10 4/128ГБ Sea Blue</p>
                <a href="#" className="btn">Купить</a>
              </div>
            </div>
            <div className="col-sm-6 col-md-4 col-lg-3 productCol">
              <div className="productCard p-3">
                <img className="d-block productImg" src="https://olcha.uz/image/200x200/products/OlO1yD3pLX3Gtmr4M4OxyulsWnzoJXDvPYz8nQGv2rNMBTlA7NzBgGY3m6vN.jpeg" alt="" />
                <a href="#" className="groupName d-block mb-2 mt-3">Смартфоны</a>
                <p className="price mb-2">2  290  000 сум  </p>
                <p className="priceMonth">222  000 сум / мес.</p>
                <p className="title">Xiaomi Redmi 10 4/128ГБ Sea Blue</p>
                <a href="#" className="btn">Купить</a>
              </div>
            </div>

          </div>
        </div>
      </section>
    </div>
  );
}

export default App;
